    ...
    const fetchUsersGraphQL = async (queryTerm: string) => {
      ...
      try {
        ...
        if (res.data && res.data.data && res.data.data.users) {
          ...
          console.warn(
            'GraphQL response unexpected structure or empty users from search:',
            res.data
          );
          setUsers([]);
        }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (form.type === 'manutenzione') {
        payload.maintenanceStart = form.maintenanceStart ? new Date(form.maintenanceStart).toISOString() : undefined;
        payload.maintenanceEnd = form.maintenanceEnd ? new Date(form.maintenanceEnd).toISOString() : undefined;
      }
      ...
      <form onSubmit={handleSubmit}>
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <LocalizationProvider dateAdapter={AdapterDateFns}>
            <DatePicker
              ...
              onChange={(newValue) =>
                setForm((prev) => ({
                  ...prev,
                  date: newValue ? newValue.toISOString().slice(0, 10) : '',
                }))
              }
              slotProps={{
                textField: {
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} label="Assegnata a" required fullWidth />
            )}
