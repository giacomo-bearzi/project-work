...
export const EditTaskModal: React.FC<EditTaskModalProps> = ({
  ...
  const [selectedAssignedUserLocal, setSelectedAssignedUserLocal] =
    useState<AutocompleteUser | null>(null);
  ...
  useEffect(() => {
    ...
    if (autocompleteInput.trim() !== '') {
      if (
        !selectedAssignedUserLocal ||
        autocompleteInput.toLowerCase() !== selectedAssignedUserLocal.fullName.toLowerCase()
      ) {
      ...
      if (editingTask && editingTask.assignedTo) {
        if (
          !selectedAssignedUserLocal ||
          (isAutocompleteUser(editingTask.assignedTo) &&
            selectedAssignedUserLocal._id !== editingTask.assignedTo._id)
        ) {
          const userId = isAutocompleteUser(editingTask.assignedTo)
  ...
  const canEditCurrentTask =
    editingTask && (currentUserRole === 'manager' || currentUserRole === 'admin');
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    try {
      // Convert date fields to ISO UTC strings
      const dateIso = editingTask.date ? new Date(editingTask.date).toISOString() : new Date().toISOString();
      const payload: any = {
        date: dateIso,
        lineId: editingTask.lineId,
        description: editingTask.description,
        assignedTo: isAutocompleteUser(editingTask.assignedTo)
          ? editingTask.assignedTo._id
          : editingTask.assignedTo,
      ...
      if (editingTask.type === 'manutenzione') {
        payload.maintenanceStart = editingTask.maintenanceStart ? new Date(editingTask.maintenanceStart).toISOString() : undefined;
        payload.maintenanceEnd = editingTask.maintenanceEnd ? new Date(editingTask.maintenanceEnd).toISOString() : undefined;
      }
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <Autocomplete
            ...
            getOptionLabel={(option) =>
              option && typeof option === 'object' ? `${option.fullName} (${option.username})` : ''
            }
              ...
              <ListItem key={item.id || idx} disableGutters>
                ...
                <ListItemSecondaryAction>
                  <IconButton
                    ...
                    disabled={editingTask.checklist.length === 1 || !canEditCurrentTask}
                  >
                    <DeleteIcon />
                  </IconButton>
                </ListItemSecondaryAction>
