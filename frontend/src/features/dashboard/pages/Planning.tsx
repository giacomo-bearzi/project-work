...
import { TaskModal } from '../../dashboard-tasks/components/TaskModal';
import { EditTaskModal } from '../../dashboard-tasks/components/EditTaskModal';
...
export const Planning = () => {
    ...
    api
      ...
      .then(async (res) => {
        ...
        const toUpdate = tasksFetched.filter(
          (t: Task) => t.status !== 'completata' && t.date < today
        );
  ...
  const handleDragEnd = async (result: any) => {
    ...
    setTasks((prev) =>
      prev.map((t) =>
        t._id === taskId ? { ...t, checklist: newChecklist, status: newStatus, completedAt } : t
      )
    );
