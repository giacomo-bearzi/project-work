import { Box, CircularProgress, Grid, Paper, Stack, useTheme, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Avatar, Chip, Button } from '@mui/material';
...
export const Issues = () => {
    ...
    >
        ...
        >
            ...
            >
                <Header />
                <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2} sx={{ mb: 2, mt:2, p:2 }}>
                  {/* Titolo a sinistra */}
                  ...
                  <Stack direction="row" spacing={2} alignItems="center">
                    ...
                    <Select
                      ...
                      onChange={e => setSelectedPriority(typeof e.target.value === 'string' ? e.target.value.split(',') : e.target.value)}
                    ...
                    <Select
                      ...
                      displayEmpty
                      value={selectedLine}
                      onChange={e => setSelectedLine(typeof e.target.value === 'string' ? e.target.value.split(',') : e.target.value)}
                      renderValue={()=> "Linea" }
                      size="small"
                      sx={{ minWidth: 120 }}
                    >
                    ...
                    <Select
                      ...
                      displayEmpty
                      value={selectedType}
                      onChange={e => setSelectedType(typeof e.target.value === 'string' ? e.target.value.split(',') : e.target.value)}
                      renderValue={()=> "Tipo"}
                      size="small"
                      sx={{ minWidth: 120 }}
                    >
                    ...
                    <TableBody>
                      {filteredIssues.map((issue, idx) => (
                        <TableRow key={issue._id}>
                          ...
                          <TableCell>
                            {issue.status === 'aperta' ? (
                              <Chip label="Aperta" sx={{ background: '#E6FAF0', color: '#00B67A', fontWeight: 600,borderColor: '#00B67A',borderWidth: 1,borderStyle: 'solid' }} />
                            ) : (
                              <Chip label="Risolta" sx={{ background: '#E6F0FA', color: '#3B82F6', fontWeight: 600,borderColor: '#3B82F6',borderWidth: 1,borderStyle: 'solid' }} />
                            )}
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            {issue.assignedTo?.fullName ? (
                              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                <Avatar sx={{ width: 24, height: 24, fontSize: 14, mr: 1 ,backgroundColor: theme.palette.mode === 'dark' ? '#fff' : 'black',
                                    color: theme.palette.mode === 'dark' ? 'black' : '#fff',}}>
                          ...
                          <TableCell>
                            {issue.resolvedAt ? moment(issue.resolvedAt).format('YYYY-MM-DD HH:mm') : '-'}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
